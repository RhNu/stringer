...
- 高置信翻译记忆可以自动填充 `translated_text`；模糊匹配只作为候选提示。
...
- 不让 annotations 或 diagnostics 影响 `import` 写回语义。
- 不因新增 annotations、diagnostics 或 pipeline 元信息升级 translation package schema version。当前软件尚未发布，schema version 只在真正需要破坏兼容边界时变更。
...
- 读写 package annotations、diagnostics 和 translated text。
...
`import_translations` 只读取 `id` 和 `translated_text`。它应忽略 annotations、diagnostics 和 pipeline metadata，避免提示数据影响写回链路。
...
- `annotations` 和 `diagnostics` 均为可选字段。
- `annotate_translations` 可重复运行。
- Pipeline-owned annotations 可替换旧值。
- Agent 写入的 `translated_text` 不应被 annotate 覆盖，除非调用方显式允许 memory auto-fill。
- `validate_translations` 重新计算 diagnostics。
- `import_translations` 忽略 annotations、diagnostics 和 pipeline metadata。
...
Agent edits translated_text in JSONL rows
...
- annotate writes annotations without changing package schema version.
...
- import ignores annotations and diagnostics.
...
- `PipelineAnnotation` 至少包含 `kind`、`id`、`layer`、`confidence`、`match` 和 processor-specific payload。
- `PipelineDiagnostic` 至少包含 `severity`、`code`、`message`、`entry_id`、`layer` 和可选 `rule_id`。
