...
- 不让 hints 或 diagnostics 影响 workspace finalize 写回语义。
...
```text
stringer workspace open --root ... --workspace translations
...
stringer workspace finalize --root ... --workspace translations --override-root ...
```
...
`validate` 只报告，不默认阻止 workspace finalize。后续可在 finalize 增加显式参数：
...
- workspace finalize ignores hints and diagnostics.
