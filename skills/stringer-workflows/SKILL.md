...
- Preserve `id`, `source`, `context`, `hints`, and `diagnostics`. Write translated text through `translation`; use `action: "skip"` with a supported `skip_reason` only for entries that do not need translation.
